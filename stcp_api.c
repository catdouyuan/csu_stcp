 ...
 *
 * 
 * 该函数用于​​阻塞等待​​，直到满足以下条件之一：
​​网络数据到达​​（从远端接收数据）
​​应用程序数据可发送​​（本地应用调用发送接口）
​​应用程序请求关闭连接​​（如调用 close()）
如果 abstime 参数为 NULL，则无限期阻塞，直到数据到达或事件发生；否则，在指定的绝对时间（abstime）超时后返回。
​​返回值​​
返回一个​​位向量（bit vector）​​，表示哪些事件已就绪（与传入的 flags 格式相同）。例如：
如果 flags 包含 NETWORK_READY 和 APP_READY，则返回值可能为 NETWORK_READY（表示网络数据可读）或两者的组合（NETWORK_READY | APP_READY）。
 */
//
...
ssize_t stcp_network_recv(mysocket_t sd, void *dst, size_t max_len)
{
     ...
     * this implementation.
     */
    printf("stcp_network_recv: len = %d\n", len);
    assert(len <= 0 ||
